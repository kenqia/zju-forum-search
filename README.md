...
搜索按波次推进。首个波次让每个首轮检索词各取一页，再调用反馈模型；后续波次先执行新检索词，然后公平推进已有检索词的分页。同一检索词在一个波次中最多请求一页。首次请求和分页共用同一个站点请求上限，也沿用来源的请求间隔、游标保护和时间提前停止规则。
...
最终列表重排只发送原始查询、运行内临时键、原生标题、作者、发布时间、板块和回复数。它不发送相关性等级、检索支持、必须概念命中、检索词数量、来源位次、轮次、URL、正文、回帖或认证数据；朵朵正文派生标题发送为空。planner 接收前和最终序列化时都会执行字段白名单。模型超时、网络失败、非法 JSON 或无法识别的响应结构不会改变本地预排序，重复键和未知键会被忽略，遗漏的 Top-M 候选按本地顺序追加。

首轮模型调用会发送用户查询和浏览器当前日期。反馈轮从待判断候选中选择有限证据，一次返回相关性等级、后续检索词和停止判断。候选按最新独立证据所属检索词轮转，每个检索词最多提供 4 条；未选中的候选仍可进入后续反馈。只有相关性等级 0 会进入默认折叠的软隔离区域，相关性等级 1、2、3 和未判断候选留在主列表。候选被不同检索词再次命中或白名单元数据变化后可以重判；同一检索词下只有位次变化不会触发重判。
...
反馈模型只收到运行内临时键、最多 3 个不同检索支持，以及标题、作者、发布时间、板块和回复数。完整负载不超过 4000 UTF-8 字节。URL、平台标识、来源位次、轮次、片段、正文、回帖、认证数据和用户身份都不会进入负载，正文派生标题发送为空。所有模型调用都关闭思考模式，将最大输出限制为 1200 tokens，并通过 `AbortController` 在 20 秒后终止。反馈超时、网络错误或无效响应会停止迭代，保留未软隔离结果，并跳过最终列表重排。
