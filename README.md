...
CC98 请求间隔为 2 秒，朵朵初始间隔为 500ms；两者默认每页请求 20 条，一次搜索按用户设置最多调用 1 至 100 次站点请求，分页同样计数。朵朵的间隔不是实测安全阈值，遇到验证码或限流会停止。达到请求上限、模型判断停止、没有新证据、用户手动停止或出错后，面板保留已有结果并说明原因。关闭面板不会停止搜索；“停止并查看结果”会终止站点请求，并让至少两条已有主结果候选进入最终列表重排。新查询会替换旧查询并取消未完成的重排。
