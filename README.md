...
CC98 请求间隔为 2 秒，朵朵初始间隔为 500ms；两者默认每页请求 20 条，一次搜索按用户设置最多调用 1 至 100 次站点请求，分页同样计数。朵朵的间隔不是实测安全阈值，遇到验证码或限流会停止。达到请求上限、模型判断停止、没有新证据、用户手动停止或出错后，面板保留已有结果并说明原因。关闭面板不会停止搜索；“停止并查看结果”会终止站点请求并让已有候选进入最终筛选；新查询会替换旧查询并取消未完成的筛选。
...
搜索结束后，扩展默认对已召回候选做最终意图筛选：按每批最多 160 个、约 4000 字节拆批，逐批把原始查询和批次临时键连同白名单元数据发给模型，模型只返回要移除的临时键。全部批次成功后才一次性移除对应结果；任何一批失败、超时或被取消都保留筛选前的完整列表并提示筛选未完成。零结果的搜索和被新查询替换的旧搜索不进入筛选；该开关可在模型设置中关闭。

首轮模型调用会发送用户查询和浏览器当前日期。反馈轮从待判断候选中选择有限证据，一次返回相关性等级、后续检索词和停止判断。候选按最新独立证据所属检索词轮转，每个检索词最多提供 4 条；未选中的候选仍可进入后续反馈。只有 grade 0 会进入默认折叠的软隔离区域，grade 1、grade 2、grade 3 和未判断候选留在主列表。候选被不同检索词再次命中或白名单元数据变化后可以重判；同一检索词下只有位次变化不会触发重判。

浏览器先执行时间硬约束，再对主列表做确定性的本地预排序。grade 3 排在 grade 2 前，grade 1 与未判断候选同级。同一等级内依次比较排除词、必须概念、不同检索词命中数、来源最佳位次和首次命中轮次。模型阶段关闭、取消或失败时，界面完整保留这个顺序。

反馈模型只收到运行内临时键、最多 3 个不同检索支持，以及标题、作者、发布时间、板块和回复数。完整负载不超过 4000 UTF-8 字节。URL、平台标识、来源位次、轮次、片段、正文、回帖、认证数据和用户身份都不会进入负载，正文派生标题发送为空。所有模型调用都关闭思考模式，将最大输出限制为 1200 tokens，并通过 `AbortController` 在 20 秒后终止。反馈超时、网络错误或无效响应会停止迭代，保留未软隔离结果，并跳过后续模型阶段。
